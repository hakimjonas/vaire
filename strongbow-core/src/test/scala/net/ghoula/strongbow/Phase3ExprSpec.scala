...
class Phase3ExprSpec extends AnyFlatSpec with Matchers {
  ...
  "ArraySize" should "return the size of an array" in {
    ...
    val results = (0 until 3).map(i => ExprInterpreter.eval(expr, arrColumns, RowIndex(i)))
  ...
  "ArraySort" should "sort array elements" in {
    ...
    results shouldBe Seq(Right(Seq(1, 2, 3)), Right(Seq(4, 5)))
  }

  "ArrayDistinct" should "remove duplicates from array" in {
    val expr = arrCell.arrayDistinct
    val result = ExprInterpreter.eval(expr, arrColumns, RowIndex(2))
  ...
  "MapKeys" should "return all keys of a map" in {
    ...
    val result = ExprInterpreter.eval(expr, mapColumns, RowIndex(0))
    result.map(_.toSet) shouldBe Right(Set("a", "b"))
  }

  "MapValues" should "return all values of a map" in {
    val expr = mapCell.mapValues
    val result = ExprInterpreter.eval(expr, mapColumns, RowIndex(0))
    result.map(_.toSet) shouldBe Right(Set(1, 2))
  }
  ...
  "MapContainsKey" should "check if map contains a key" in {
    ...
    ExprInterpreter.eval(exprTrue, mapColumns, RowIndex(0)) shouldBe Right(true)
    ExprInterpreter.eval(exprFalse, mapColumns, RowIndex(0)) shouldBe Right(false)
  }

  "MapEntries" should "return key-value pairs as a sequence of tuples" in {
    val expr = mapCell.mapEntries
    val result = ExprInterpreter.eval(expr, mapColumns, RowIndex(0))
    result.map(_.toSet) shouldBe Right(Set(("a", 1), ("b", 2)))
  }
