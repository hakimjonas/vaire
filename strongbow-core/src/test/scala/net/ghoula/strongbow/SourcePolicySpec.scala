...
class SourcePolicySpec extends AnyFlatSpec with Matchers {
  ...
  private val repoRoot: java.io.File = {
    ...
    chain
      .find(c =>
        java.nio.file.Files.exists(c.resolve("strongbow-core/src/main")) &&
          java.nio.file.Files.exists(c.resolve("strongbow-spark/src/main"))
      )
      .get
      .toFile
  }
  ...
  private def mainSources: Vector[java.io.File] = {
    val roots = Vector(
      java.nio.file.Paths.get(repoRoot.getPath, "strongbow-core/src/main"),
      java.nio.file.Paths.get(repoRoot.getPath, "strongbow-spark/src/main")
    )
    roots.flatMap { root =>
