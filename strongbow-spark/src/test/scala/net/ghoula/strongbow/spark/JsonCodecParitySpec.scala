...
/** Parity for `from_json` / `to_json`.
  ...
  *
  * Divergence (documented, not tested as parity): for a malformed JSON string, Strongbow's typed
  * decode yields NULL, while Spark's permissive `from_json` yields an all-null struct (which
  * `to_json` serializes as `{}`). Spark's null-struct has no faithful Scala counterpart, so
  * Strongbow chooses NULL.
  */
class JsonCodecParitySpec extends AnyFlatSpec with Matchers with SparkTestBase {
  ...
  "from_json" should "yield NULL for malformed JSON (Strongbow semantics)" in {
