package net.ghoula.vaire.column
...
enum Column[+A] {
  ...
  private[vaire] inline def nullSet: BitSet = this match {
...
object Column {
  ...
  private[vaire] def structLength(columns: Vector[Column[?]]): Int =
    columns.headOption.fold(0)(_.length)

  private[vaire] def arrayColumnType[A](elements: Column[A]): ColumnType =
    ColumnType.ArrayType(elements.columnType)

  private[vaire] def mapColumnType[K, V](keys: Column[K], values: Column[V]): ColumnType =
    ColumnType.MapType(keys.columnType, values.columnType)

  private[vaire] def arrayValueAt(elements: Column[?], offsets: Array[Int], index: Int): Any | Null =
    (offsets(index) until offsets(index + 1)).map(elements.getValue).toSeq

  private[vaire] def mapValueAt(keys: Column[?], values: Column[?], offsets: Array[Int], index: Int): Any | Null =
    (offsets(index) until offsets(index + 1))
      .map(j => keys.getValue(j) -> values.getValue(j))
      .toMap

  private[vaire] def structValueAt[T](
  ...
  private def emptyStructSchemaOf[X](fields: Vector[(String, ColumnType)]): Schema[X] = new Schema[X] {
    ...
    def decode(values: Vector[Any]): Either[net.ghoula.vaire.errors.DecodeError, X] =
      Left(net.ghoula.vaire.errors.DecodeError.TypeMismatch("Struct", "empty struct column"))
  }
