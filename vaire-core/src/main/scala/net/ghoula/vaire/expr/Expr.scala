...
import net.ghoula.vaire.types.{Binary, ColumnIndex, Date, DayTimeInterval, Decimal, Time, Timestamp, YearMonthInterval}
...
enum Expr[Row, +A] {
  ...
  /** Spark-only: grouping indicator for GROUPING SETS / ROLLUP / CUBE.
    *
    * Deliberate deviation: Vairë's model has no grouping-sets context, so the in-memory interpreter
    * returns Left(Unsupported).
    */
  ...
  /** Spark-only: bit vector of grouping indicators over the given columns.
    *
    * Deliberate deviation: requires a grouping-sets context absent from Vairë's model; in-memory
    * returns Left(Unsupported).
    */
