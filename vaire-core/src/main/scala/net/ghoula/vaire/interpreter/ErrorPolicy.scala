...
/** The result of a quarantine evaluation: the value column with failed rows null-marked, and the
  * dense per-row error column — an [[net.ghoula.vaire.column.Column.AnyColumn AnyColumn]] boxing
  * [[QuarantinedRow]] at every failed row, null where the row succeeded. Unlike the Collect list,
  * the error column is complete: exactly one entry per failed row, with no bound, so dead-letter
  * routing never silently drops reasons.
  */
