...
import net.ghoula.vaire.expr.{Binder, Expr}
import net.ghoula.vaire.types.{Date, DayTimeInterval, RowIndex, Time, YearMonthInterval}
...
object ExprInterpreter {
  ...
  private[vaire] def inferExprColumnType[Row, A](expr: Expr[Row, A], columns: Vector[Column[?]]): ColumnType = {
  ...
  /** A typed column of the given flat element values, used as a lambda binding.
    ...
    * placeholder value. Heterogeneous element types (outside Vairë's data model) therefore surface
    * as nulls, and a body whose type arm has no sample match yields an empty column — the
