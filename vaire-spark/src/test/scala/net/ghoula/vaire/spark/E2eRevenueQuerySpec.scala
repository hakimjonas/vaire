...
/** TPC-H Q6 proof-of-concept: vaire Dataset plan vs native Spark SQL.
  *
  ...
  * this is fully expressible in vaire using distributed Expr-based operations.
  */
class E2eRevenueQuerySpec extends AnyFlatSpec with Matchers with SparkTestBase with SlowTests {
  ...
  "TPC-H Q6" should "produce identical results via vaire and native Spark" in {
    ...
    info(f"Vairë: revenue = $sbRevenue%.2f (${sbValues.size} rows, ${(t1 - t0) / 1e6}%.1f ms)")
  ...
  it should "show identical Spark physical plans" in {
    ...
    info(s"Vairë Q6 result rows: ${result.toOption.get.toVectorUnsafe.size}")
  }
}
