...
/** Synthetic TPC-H data generators for proof-of-concept queries.
  ...
  * seeded PRNG for reproducibility. Field types follow TPC-H but use Double for DECIMAL (vaire has
  * no BigDecimal) and String for DATE (ISO-8601 format).
  */
object E2eTestData {
  ...
  /** Build a vaire Dataset from LineItem rows. */
  ...
  /** Build a vaire Dataset from Part rows. */
