...
/** json_tuple representation benchmark — the measurement behind the representation decision.
  ...
  *     that composes with Vairë's single-projection `df.select(columns*)` model.
  *
  ...
  *   retains 10-45% less but needs a dynamic-shape Schema (foreign to Vairë's compile-time
  *   typed struct model).
  ...
  *   Native json_tuple parses once (1-3.5x faster at n keys) but cannot compose with Vairë's
...
class JsonTupleBench extends AnyFlatSpec with Matchers with SparkTestBase {
  ...
  private def evalStruct(jsonData: Array[String | Null], keys: Vector[String]): Column[?] = {
    ...
    val dynSchema: Schema[Vector[Any | Null]] = new Schema[Vector[Any | Null]] {
      ...
      def decode(values: Vector[Any]): Either[net.ghoula.vaire.errors.DecodeError, Vector[Any | Null]] =
        Right(values)
    }
    Column.struct(
